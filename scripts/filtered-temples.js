...
function menuOptionSelected(option) {
    ...
    pageHeading.innerHTML = `<h2>${menuOptions[option].headingName}</h2><p>${menuOptions[option].description}</p>`;
...
function displayTemples(templesToDisplay) {
    ...
    templesToDisplay.forEach(temple => {
        ...
        details.innerHTML = `<strong>Location:</strong> ${temple.location}<br><strong>Dedicated:</strong> ${temple.dedicated}<br><strong>Size:</strong> ${temple.area} sq ft`;
    ...
    switch (option) {
        case "old":
            filteredTemples = temples.filter(temple => {
                const year = parseInt(temple.dedicated.split(", ")[0]); // Extract year from the dedicated date
                return year < 1900;
            });
            break;
        case "new":
